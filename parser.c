...
parse_if_statement(void)
{
	...
	if(nextsym.sym == SYM_ELSE){
	  //codegen_put_comment("else文",1);
	  codegen_put_code_str("jt",l2);
...
parse_while_statement(void)
{
	...
	//codegen_put_comment("while文終わり",1);
	codegen_put_label(l2);
}
...
parse_do_statement(void)
{
	...
	codegen_put_label(l1);
	//codegen_put_comment("do_while文",1);
	nextsym = scanner_get_next_sym();
	parse_statement();
	...
	//codegen_put_comment("do_while文終わり",1);
}
